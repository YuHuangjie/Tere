...
#include "image/jpeg/turbojpeg.h"
 ...
 */
bool DecodeJpeg(const string &filename, unsigned char *buf, const unsigned int bufsize,
...
*/
bool DecodeJpegHeader(const string &filename, unsigned int &width, unsigned int &height)
{
	ifstream in_file(filename, fstream::binary);
	unsigned int jpeg_size;
	unsigned char *jpeg_buf = nullptr;
