...
//Math types derived from the KempoApi tMath library
    ...
    typedef union Matrix3f_t
    {
			...
			Matrix3f_t& T() {
				for (int i = 0; i < 9; ++i)
					std::swap(this->M[i], this->M[(i % 3)*3 + i/3]);
    ...
    typedef union Matrix4f_t
    {
			...
			Matrix4f_t& T() {
				for (int i = 0; i < 16; ++i)
					std::swap(this->M[i], this->M[(i % 4) * 4 + i / 4]);
...
//Math functions
		...
		static void Matrix4fSetZero(Matrix4fT* NewObj)
	{
		...
		NewObj->s.M03 = NewObj->s.M13 = NewObj->s.M23 =
			NewObj->s.M30 = NewObj->s.M31 = NewObj->s.M32 =
			NewObj->s.M33 = 0.0f;
	}
	...
	*/
		...
		static void Matrix4fSetIdentity(Matrix4fT* NewObj)
	{
		...
		NewObj->s.M00 =
			NewObj->s.M11 =
			NewObj->s.M22 = NewObj->s.M33 = 1.0f;
	}
    /**
